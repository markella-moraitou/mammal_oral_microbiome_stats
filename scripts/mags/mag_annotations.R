...
# DRAM output
bac_dram <- read.table(file.path(indir, "MAG_annotations.tsv"),
...
if(file.exists(file.path(subdir, "GIFTs.csv"))) {
  ...
  GIFTs <- distill(bac_dram, GIFT_db, genomecol=2, annotcol=c(9, 10, 20, 21, 22))
...
GIFTs_elements_long <- 
  ...
  inner_join(select(meta_tips, c(label, bin, domain:species, phylum_grouped, host_species, host_common.name, host_order, calculated_cluster_main_diet)), by=c("Sample"="bin")) %>%
...
p <- ggtree(bac_tree, layout = "circular", size = 1.5) %<+% select(bac_meta, c(label, host_order, habitat.general, calculated_cluster_main_diet)) +
  ...
  geom_tippoint(shape=21, size=3, stroke=1, 
                aes(fill=host_order, color=calculated_cluster_main_diet)) +
...
#### Archaea ####
# Colour by host order and diet
p <- ggtree(ar_tree) %<+% select(ar_meta, c(label, host_order, habitat.general, calculated_cluster_main_diet)) +
...
p <- ggtree(strep_tree) %<+% select(bac_meta, c(label, host_order, calculated_cluster_main_diet)) +
  geom_tiplab(size=3, aes(colour=host_order)) +
  scale_colour_manual(values = order_palette, name = "Host order", na.value = "black") +
  new_scale_color() +
  geom_tippoint(shape=21, size=3, stroke=1, 
...
# Colour by order and habitat
p <- ggtree(actino_tree) %<+% select(bac_meta, c(label, host_order, calculated_cluster_main_diet)) +
  geom_tiplab(size=3, aes(colour=host_order)) +
  scale_colour_manual(values = order_palette, name = "Host order", na.value = "black") +
  new_scale_color() +
  geom_tippoint(shape=21, size=3, stroke=1, 
...
# Colour by order and habitat
p <- ggtree(rothia_tree) %<+% select(bac_meta, c(label, host_order, calculated_cluster_main_diet)) +
  geom_tiplab(size=3, aes(colour=host_order)) +
  scale_colour_manual(values = order_palette, name = "Host order", na.value = "black") +
  new_scale_color() +
  geom_tippoint(shape=21, size=3, stroke=1, 
...
# Colour by order and habitat
p <- ggtree(neisseria_tree) %<+% select(bac_meta, c(label, host_order, calculated_cluster_main_diet)) +
  geom_tiplab(size=3, aes(colour=host_order)) +
  scale_colour_manual(values = order_palette, name = "Host order", na.value = "black") +
  new_scale_color() +
  geom_tippoint(shape=21, size=3, stroke=1, 
