...
# Calculate host distances
host_dist <- cophenetic(host_consensus) 
...
parafit_result_ph <- parafit(host.D = host_dist, para.D = mb_dist_clr_matrix, HP = assoc, nperm = 1000, seed = 123)
...
parafit_result_d <- parafit(host.D = diet_sp_dist_matrix, para.D = mb_dist_clr_matrix, HP = assoc, nperm = 1000, seed = 123)
...
for (i in 1:n_iter) {
  ...
  links_sampled <- links %>% filter(phy2 %in% sampled_mbs)
  ...
  # Run parafit
  res <- parafit(host.D = host_dist, para.D = mb_dist_clr_matrix_sampled, HP = assoc_sampled, nperm = 1000)
