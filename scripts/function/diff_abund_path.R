...
subdir <- normalizePath(file.path(outdir, "diff_abund_pathways"))
...
otu_table <- as.data.frame(phy_pathway_clr@otu_table)
sample_data <- as.data.frame(phy_pathway_clr@sam_data)
...
# Keep only most abundant pathways (first average by species to weight against uneven sampling, then average over species)
# Note: Currently this retains all pathways as there are less than 200
...
for (path in pathways) {
    cat("Running PGLMM for pathway", which(pathways == path), "of", length(pathways), "--", path, "\n")
...
for (path in pathways) {
    cat("Calculating phylogenetic signal for pathway", which(pathways == path), "of", length(pathways), "--", path, "\n")
...
combined_res <- phy_res %>% rename(coef_lambda = lambda,
    ...
    # Add metadata
    left_join(unique(rownames_to_column(data.frame(phy_pathway_clr@tax_table), "pathway")))
...
write.csv(combined_res, file = file.path(subdir, "pglmm_pagel_combined_results.csv"), quote = TRUE, row.names = FALSE)
...
common_categories <- table(combined_long$category) %>% sort(decreasing = TRUE) %>% head(8) %>% names
...
cat_palette <- brewer.pal(n = length(unique(combined_long$category))-1, name = "Dark2")
names(cat_palette) <- unique(combined_long$category)[-length(unique(combined_long$category))] # Remove "Other" from names
cat_palette["Other"] <- "black"

# Plot
p <- ggplot(combined_long, aes(x = coefficient, y = category, colour = category, shape = significant)) +
    geom_point(alpha = 0.7, size = 1.5, position = position_jitter(height = 0.2)) +
    geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
    facet_grid(cols = vars(term), scales = "free_x") +
    scale_colour_manual(values = cat_palette, name = "Pathway class") +
...
ggsave(p, filename = file.path(subdir, "pglmm_pagel_combined_pathways.png"), width = 8, height = 6)
...
if (file.exists(file.path(subdir, "mcmcglmm_output.RDS"))) {
    ...
    prior = list(R = list(V = diag(1), nu = 0.002),
                 G = list(G1 = list(V = diag(1), nu = 0.002, alpha.mu = rep(0,1), alpha.V = diag(1)*100),
                          G2 = list(V = diag(n_paths), nu = 0.002, alpha.mu = rep(0, n_paths), alpha.V = diag(n_paths)*100)))
    start.time <- Sys.time()
...
random_results <- summary(m1)$Gcovariances %>%
  data.frame %>% rownames_to_column("term") %>%
  mutate(pathway = str_remove(term, ".Species")) %>% # Separate pathway
  mutate(term = str_remove(term, paste0(pathway, "."))) %>%  # Separate term
...
write.csv(lambda, file = file.path(subdir, "mcmcglmm_lambda_pathway.csv"), quote = FALSE, row.names = FALSE)
...
r2 <- data.frame(pathway = lambda$pathway, R2 = sapply(lambda$pathway, get_r2, model = m1))
